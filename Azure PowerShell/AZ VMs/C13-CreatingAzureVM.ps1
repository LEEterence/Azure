...
# 4. Creating the OS Image #########################################
    ...
    Get-AzVMSize -Location 'West US'
        ...
        Get-AzComputeResourceSku | Where-Object {$_.Locations -icontains "westus" -and $_.ResourceType.Contains("virtualMachines")}
        # Specifying the VM size
        Get-AzComputeResourceSku | Where-Object {$_.Locations -icontains "westus" -and $_.ResourceType.Contains("virtualMachines") -and $_.name -like "*B1ms*"}
