...
! REMINDERS: 
    ...
    @3. If using a Load Balancer - MUST be in the subnet of the Load Balancer
Summary: each VM will require unique:
    ...
    6. Allow user to define how many instances of VMs
*@Changes
    - IIS install functioning for loop
    - Adjusted example to install to the Load Balancer subnet

.Example Adding to Default Virtual Network & Subnet
New-IISWebServers -ResourceGroupName sleepygeeks -VmName SG-AZWEB -HostName SG-AZWEB -SubnetName sleepygeeks-subnet -VirtualNicName sg-webnic -VirtualNetworkName sleepygeeks-vnet -avs sleepygeeks-availabilityset-iis -AdminCredential azureuser -verbose

.Example Adding to Load-Balancer BACKEND subnet with Bastion support (Names are different - NOTE: Bastion has to be setup already)
New-IISWebServers -ResourceGroupName sleepygeeks -VmName SG-AZWEB -HostName SG-AZWEB -SubnetName sleepygeeks-lb-backendsubnet -VirtualNicName sg-webnic -VirtualNetworkName sleepygeeks-lb-vnet -avs sleepygeeks-availabilityset-iis -AdminCredential azureuser -Verbose
...
function Show-Progress {
    ...
    while($runningjobs -gt 0) {
        ...
        $Progress = [System.Math]::Round((($total-$runningjobs)/$total*100),2)
...
# Might have to add the required parameters
...
function New-IISWebServers {
    ...
    for ($x = 1;$x -lt 3;$x++){
        if (Get-AzVm -Name "$VmName$x" -ResourceGroupName $ResourceGroupName -ErrorAction Ignore) {
            ...
            New-AzVM -VM $vm -ResourceGroupName $ResourceGroupName -Location $Location -AsJob | Format-Table -AutoSize
    ...
    #Cleanup
    $vmprogress | Where-Object {$_.State -eq "Completed"} | Remove-Job
